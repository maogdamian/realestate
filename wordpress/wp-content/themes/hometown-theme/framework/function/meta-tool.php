...
class nt_metaboxes_tool {
	...
	function create_metaboxes() {
		$this->nt_input_tool = new nt_input_tool( $this->options, $this->config );
	...
	function metabox_callback() {
		global $post;
		if($this->config['context'] == 'side') $this->nt_input_tool->generate_meta_option_for_side();
		else $this->nt_input_tool->generate_meta_option();
