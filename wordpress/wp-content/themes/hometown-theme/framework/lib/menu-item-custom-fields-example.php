...
class Menu_Item_Custom_Fields_Example {
	...
	protected static $fields_spec = array();
	...
	public static function init() {
		...
		self::$fields_spec = array(
			'mega-menu' => array(
				'type' => 'select',
				'option' => array(
					'' => 'None',
					'mega-menu mega-menu-2col' => '2 Columns',
					'mega-menu mega-menu-3col' => '3 Columns',
					'mega-menu mega-menu-4col' => '4 Columns',
				)
			),
			'style' => array(
				'type' => 'select',
	...
	public static function _save( $menu_id, $menu_item_db_id, $menu_item_args ) {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && !defined('DOING_AJAX_IMPORT') ) {
			return;
		}
		...
		foreach ( self::$fields as $_key => $label ) {
			...
			else if ( ! empty ($menu_item_args[$key]) ) {
				$value = $menu_item_args[$key];
			}
			else {
				$value = null;
			}
		...
		foreach ( self::$fields as $_key => $label ) :
			...
			?>
				<p class="description description-wide <?php echo esc_attr( $class ) ?>">
					...
					<?php if(self::$fields_spec[$_key]['type'] == 'text') printf(
					...
					<?php if(self::$fields_spec[$_key]['type'] == 'select') printf(
						'<label for="%1$s">%2$s<br /><select id="%1$s" class="widefat %1$s %4$s" name="%3$s">',
						esc_attr( $id ),
						esc_html( $label ),
						esc_attr( $name ),
						$key
					); 
						foreach(self::$fields_spec[$_key]['option'] as $option_key => $option_value) {
							if($value == $option_key) {
