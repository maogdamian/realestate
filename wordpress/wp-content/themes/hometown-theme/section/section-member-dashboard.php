...
<?php
$all_properties = get_posts(array('post_type' => 'property', 'post_status' => 'any', 'author' => $current_user->ID, 'posts_per_page' => -1));
$pages_quantity = ceil(count($all_properties)/$post_per_page);
for($i=1; $i<=$pages_quantity; $i++): ?>
	<?php
		if($i == get_query_var('paged') || ($i==1 && get_query_var('paged') == 0)) {
